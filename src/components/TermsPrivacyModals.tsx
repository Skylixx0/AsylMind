...
import { PolicySection, TermsOfUseItem } from '@/src/components/lib/types';

export default function TermsPrivacyModals() {
  const t = useTranslations("Home.modal");
  const terms = t.raw('termsOfUse.terms');
  const privacy = t.raw('privacyPolicy.privacy');
    ...
    <div className="flex flex-col md:flex-row items-center gap-4 text-sm text-muted-foreground">
      ...
      <Dialog open={openModal === 'terms'} onOpenChange={(open) => setOpenModal(open ? 'terms' : null)}>
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className='font-bold text-xl'>{t("termsOfUse.title")}</DialogTitle>
          </DialogHeader>
          <div className="text-sm space-y-4">
            <p>{t("termsOfUse.description")}</p>
            <p>{t("termsOfUse.lastUpdated")}</p>
            {terms.map((data: TermsOfUseItem, index: number)=>(
      ...
      <Dialog open={openModal === 'privacy'} onOpenChange={(open) => setOpenModal(open ? 'privacy' : null)}>
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className='font-bold text-xl'>{t("privacyPolicy.title")}</DialogTitle>
          </DialogHeader>
          <div className="text-sm space-y-4">
            <p>{t("privacyPolicy.lastUpdated")}</p>
            {privacy.map((data: PolicySection, idx)=>(
                !data.list ? (
